...
router.get('/:id', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        ...
        const isParticipant = session.participants.some((p) => p.user.toString() === userId);
        if (!isParticipant) {
...
// Join session
router.post('/:id/join', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
// Leave session
router.post('/:id/leave', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
        ...
        if (session.participants.every((p) => p.leftAt)) {
            ...
            session.duration = Math.floor(((session.endTime.getTime() - (session.startTime?.getTime() || Date.now())) / 1000 / 60));
        }
        await session.save();
...
// Extend session
router.post('/:id/extend', authenticate, ...validateSessionId, body('minutes').optional().isInt({ min: 1, max: 60 }).withMessage('Minutes must be between 1 and 60'), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
// Pause session
router.post('/:id/pause', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
// Resume session
router.post('/:id/resume', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
router.put('/:id', authenticate, ...validateSessionId, body('topic').optional().isString().trim(), body('goals').optional().isArray(), body('language').optional().isString().trim(), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
// Start recording
router.post('/:id/recording/start', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
// Stop recording
router.post('/:id/recording/stop', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
router.post('/:id/collaboration', authenticate, ...validateSessionId, body('type').isIn(['editor', 'terminal', 'whiteboard', 'chat', 'resource']).withMessage('Invalid collaboration type'), body('data').isObject().withMessage('Data is required'), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
        ...
        switch (type) {
            case 'editor': {
                ...
                let fileEntry = session.collaboration.editor.files.find((f) => f.name === file.name);
                if (fileEntry) {
        ...
        const engagementScore = session.analytics.totalCodeChanges * 2 +
            session.analytics.chatMessages * 1 +
...
// Get session history
router.get('/history', authenticate, query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be between 1 and 100'), query('offset').optional().isInt({ min: 0 }).withMessage('Offset must be non-negative'), query('status').optional().isIn(['completed', 'ended_early', 'reported']).withMessage('Invalid status'), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
...
router.delete('/:id', authenticate, ...validateSessionId, async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ success: false, errors: errors.array() });
