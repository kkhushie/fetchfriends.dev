...
passport.use(new GitHubStrategy({
    ...
    try {
        ...
        const repos = await VerificationService.fetchGitHubRepos(profile.username, accessToken);
        const githubScore = await VerificationService.calculateGitHubScore(githubData, repos, accessToken);
        // Find or create user
...
passport.use(new LinkedInStrategy({
    ...
    try {
        ...
        if (user) {
            ...
            const existingMethod = user.verification.methods.find((m) => m.provider === 'linkedin');
            if (!existingMethod) {
        ...
        const scores = user.verification.methods.map((m) => m.data?.score || 0);
...
router.get('/github/callback', passport.authenticate('github', { session: false, failureRedirect: '/login?error=github' }), (req, res) => {
    const user = req.user;
...
router.get('/linkedin/callback', passport.authenticate('linkedin', { session: false, failureRedirect: '/login?error=linkedin' }), (req, res) => {
    const user = req.user;
    const token = generateToken(user._id.toString());
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:3000'}/auth/callback?token=${token}`);
});
...
// Verify status
router.get('/verify/status', authenticate, async (req, res, next) => {
    try {
