    ...
    try {
        ...
        if (availability) {
            if (availability.status)
                user.availability.status = availability.status;
            if (availability.lookingFor)
                user.availability.lookingFor = availability.lookingFor;
        }
        await user.save();
        res.json({ success: true, user });
    }
...
router.post('/availability', authenticate, async (req, res, next) => {
    try {
        ...
        if (lookingFor)
            user.availability.lookingFor = lookingFor;
