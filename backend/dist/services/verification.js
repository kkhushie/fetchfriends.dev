...
export class VerificationService {
     ...
     */
    static async calculateGitHubScore(githubData, repos, accessToken) {
        let score = 0;
     ...
     */
    static calculateLinkedInScore(linkedinData) {
        let score = 0;
        ...
        const keywordMatches = techKeywords.filter((keyword) => headlineLower.includes(keyword)).length;
     ...
     */
    static async fetchGitHubRepos(username, accessToken) {
        try {
     ...
     */
    static getVerificationStatus(score) {
        if (score >= 80)
            return 'verified';
        if (score >= 60)
            return 'pending'; // Needs quiz
        if (score >= 40)
            return 'flagged'; // Needs manual review
        return 'rejected';
    }
}
