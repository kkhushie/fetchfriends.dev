...
router.post('/:sessionId', authenticate, async (req: AuthRequest, res, next) => {
  try {
    ...
    const existingFeedback = session.feedback.find(
      (f) => f.from.toString() === userId && f.to.toString() === otherParticipant.user.toString()
    );
...
// Get feedback given
router.get('/given', authenticate, async (req: AuthRequest, res, next) => {
  try {
    const userId = req.user?._id.toString();
    const { limit = 20, offset = 0 } = req.query;
